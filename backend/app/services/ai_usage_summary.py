...
def build_ai_usage_summary(rows: Iterable[Any], start: datetime, end: datetime) -> dict[str, Any]:
    ...
    groups.sort(key=lambda item: (-(item["calls"] + item["failed_calls"]), item["provider"], item["model"], item["operation"]))
    return {
        ...
        "total_failed_calls": sum(item["failed_calls"] for item in groups),
